...
import com.nurbk.ps.projectm.ui.dialog.MapDialog
...
import com.nurbk.ps.projectm.utils.Utility.getMediaPath
...
class ChatFragment : Fragment(), MapDialog.OnClickSaveLocationMap {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mBinding.btnSend.setOnClickListener {
            uploadData(text = mBinding.msgText.text.toString(), typeMessage = TYPE_MESSAGE_TEXT)
            mBinding.msgText.text.clear()

        }

        mBinding.btnLocation.setOnClickListener {
            MapDialog(this).show(requireActivity().supportFragmentManager, "")
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_IMAGE_CODE &&
            ...
            viewModel.uploadImage(compressFormat(data!!.data!!, requireActivity())) {
                uploadData(photoUrl = it, typeMessage = TYPE_MESSAGE_IMAGE)
            ...
            viewModel.uploadVideo(data!!.data!!, "mp4", {
                uploadData(photoUrl = it, typeMessage = TYPE_MESSAGE_VIDEO)
            }, {
            ...
            val cursor: Cursor? =
                requireActivity().contentResolver.query(intent.data!!, proj, null, null, null)

            val columnIndex: Int = cursor!!.getColumnIndexOrThrow(MediaStore.Images.Media.TITLE)
            cursor.moveToFirst()


            viewModel.uploadVideo(data!!.data!!, "pdf", {
                uploadData(
                    photoUrl = it,
    ...
    private fun sendRecodingMessage(audioPath: String) {
        viewModel.uploadUri(id, user.id, audioPath) { uri ->
            uploadData(photoUrl = uri, typeMessage = TYPE_MESSAGE_RECORD)
        }
    }

    override fun getLocationMap(latLng: LatLng) {
        uploadData(text = "${latLng.latitude},${latLng.longitude}", typeMessage = TYPE_MESSAGE_MAP)
    }
    ...
    private fun getMsgAdapterListener(): MessageAdapter.MsgAdapterListener {
        return object : MessageAdapter.MsgAdapterListener {
            ...
            override fun showPdf(message: Message) {
                val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse(message.photoUrl))
